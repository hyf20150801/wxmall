/*
  1 获取用户的收货地址
    ...
    2 获取 用户 对小程序 所授予 获取地址的权限 状态 scope
     ...
     官方改了接口三种情况都返回true,不用诱导了，直接调用wx.chooseAddress就行
    ...
    4 全选的实现 数据的展示
      ...
      2 根据购物车中的商品数据 所用商品被选中 checked=true 全选就被选中
    5 总价格和总数量
      1 都要商品被选中 我们才能计算
      2 获取购物车数组
      3 遍历
    ...
    7 全选和反选
      1 全选复选框绑定事件 change
      2 获取data中的全选遍历 allChecked
      3 直接取反 allChecked=!allChecked
      4 遍历购物车数组 让里面 商品 选中状态跟随 allChecked 改变而改变
      5 把购物车数组 和 allChecked重新设置回data 把购物车重新设置回 缓存中
    8 商品数量的编辑
      1 "+" "-" 按钮 绑定同一个点击事件 区分的关键在于自定义属性
        1 "+" "+1"
        2 "-" "-1"
      ...
      4 当 购物车数量 =1同时 用户 点击 "-"
        ...
        2 取消 什么都不做
      5 直接修改商品对象的数量 num
      6 把cart数组重新设置回缓存和data中 this.setCart
    9 点击结算
      1 判断有没有收货地址信息
      2 判断用户有没有选购商品
      3 经过以上验证 调整到支付页面!
